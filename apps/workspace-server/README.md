...
Development Electron connects through the local control plane. Test Electron
waits for the server to publish `server.json`. Closing Electron leaves the
server, active conversations, and extensions running. To change workspaces,
restart the server with a different `HALO_WORKSPACE_ROOT` and reload Electron.
...
Graceful server shutdown removes both files. The local control plane reads the
latest `server.json` after a server restart. Test Electron reads that file
directly.
...
└── Electron
    ...
    ├── test: read server.json → connect over HTTP RPC
