...
`schema.ts` default-exports the Tandem schema and named-exports its relations. The SDK connects the browser client to `/sync/` and stores hosted data in `.halo/extension-data/<id>/store.json`. Never read or write that file directly.
...
Define relations after the schema so Tandem can validate collection and field names. `one` adds a many-to-one relation whose query result is one target record or `null`; `many` adds a one-to-many relation whose result is an array. In both cases, `from` names the source collection field and `to` names the target collection field. Export the result as `relations`; the generated browser entry imports that exact name.
