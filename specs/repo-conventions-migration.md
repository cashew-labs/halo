...
WorkspaceService owns note reconciliation and recovery records, borrowing the host-owned LLMApi. A merge helper isolates conflicting sections before inference. FilesystemService orders conditional saves with its normal asynchronous writes; inference runs outside that queue. The additive protocol supports existing protocol-18 clients while protocol 19 exposes reconciliation. The current protocol 20 adds thread status commands and the server continues to accept protocols 18 and 19. Existing workspace-server consumer tests exercise the public RPC and real agent file tool, including stale results, inference failures, paragraph boundaries, and recovery history.
