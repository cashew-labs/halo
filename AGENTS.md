...
Electron development mode also uses the active ADC principal as its local UI identity, so it does not open browser Google sign-in. User ADC appears as that Google user and service-account ADC appears as the service account email. Development Electron exchanges that ADC token with the local control plane and then uses the same `/workspace/health` and `/workspace/rpc` paths as production. Packaged builds and the browser app still use Better Auth with Google sign-in. Test Electron (`HALO_E2E=1`) still connects through `server.json`.

Configure the workspace when starting the workspace server with `HALO_WORKSPACE_ROOT`, or pass a JSON configuration to `pnpm server <config.json>`. Electron has no workspace picker and never starts or stops the server. In development all services use `<repo>/tmp/workspace/.halo`. The workspace server publishes `server.json` for the local control-plane gateway and `rpc.json` for the CLI. Closing Electron leaves active sessions and extensions running. See `apps/workspace-server/README.md`.
