...
the local Electron app. Development Electron then talks to the local control
plane over the same `/workspace/health` and `/workspace/rpc` paths as
production. Packaged builds and the browser app continue to use Better Auth with
Google sign-in. Electron E2Es (`HALO_E2E=1`) still discover the workspace
through `server.json`.
